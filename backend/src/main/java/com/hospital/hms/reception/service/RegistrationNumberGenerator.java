...
/**
 * Generates Registration Number in format REG-YYYY-XXXXXX. DB-independent; uses in-memory
...
public class RegistrationNumberGenerator {
    ...
    public RegistrationNumberGenerator(PatientRepository patientRepository) {
    ...
    public String generate() {
        ...
        String regNo = String.format("%s-%d-%0" + SEQ_LENGTH + "d", PREFIX, currentYear, seq);
        // Ensure uniqueness: if already exists (e.g. from DB seed), bump until unique
        while (patientRepository.findByRegistrationNumber(regNo).isPresent()) {
            seq = sequence.incrementAndGet();
            regNo = String.format("%s-%d-%0" + SEQ_LENGTH + "d", PREFIX, currentYear, seq);
        }
        return regNo;
    }
}
